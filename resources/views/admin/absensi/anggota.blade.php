...
<head>
    ...
    <title>Riwayat Absensi Anggota - ConnecTIK Admin</title>
                    ...
                    <div class="d-flex align-items-center">
                        @if($anggota->foto)
                            <img src="{{ asset('storage/anggotas/' . $anggota->foto) }}" 
                                 ...
                                 class="avatar me-3">
                        @else
                            <div class="avatar bg-white d-flex align-items-center justify-content-center me-3">
                                <i class="fas fa-user text-primary" style="font-size: 2rem;"></i>
                            </div>
                        @endif
                        <div>
                            <h2 class="mb-2">{{ $anggota->nama }}</h2>
                            <p class="mb-1">
                                <i class="fas fa-envelope me-2"></i>{{ $anggota->email }}
                                <i class="fas fa-phone ms-3 me-2"></i>{{ $anggota->no_telepon }}
                            </p>
                            <p class="mb-0">
                                <i class="fas fa-briefcase me-2"></i>{{ $anggota->jabatan }}
                                <i class="fas fa-map-marker-alt ms-3 me-2"></i>{{ $anggota->alamat }}
                            </p>
                        </div>
                    </div>
                </div>
                <div class="col-md-4 text-end">
                    <a href="{{ route('anggota.show', $anggota->id) }}" class="btn btn-light">
                ...
                <div class="card">
                    <div class="card-header">
                        <div class="d-flex justify-content-between align-items-center">
                            <h4 class="mb-0">
                                <i class="fas fa-clipboard-check me-2"></i>Riwayat Absensi
                            </h4>
                            <span class="badge bg-primary">{{ $stats['total'] }} Kegiatan</span>
                        </div>
                    </div>
                    <div class="card-body">
                        <!-- Statistics -->
                        <div class="row mb-4">
                            <div class="col-md-3">
                                <div class="stats-card">
                                    <h3>{{ $stats['total'] }}</h3>
                                    <p><i class="fas fa-clipboard-list me-2"></i>Total Kegiatan</p>
                                </div>
                            </div>
                            <div class="col-md-3">
                                    ...
                                    @foreach($absensiKegiatans as $index => $absensi)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <br><small class="text-muted">{{ $absensi->riwayatKegiatan->penyelenggara }}</small>
                                            </td>
                                            ...
                                            <td>
                                                <div class="action-buttons">
                                                    ...
                                                    <a href="{{ route('riwayat-kegiatan.show', $absensi->riwayat_kegiatan_id) }}" 
                                                       class="btn btn-success btn-sm" title="Lihat Kegiatan">
                        ...
                        @if($absensiKegiatans->isEmpty())
                            <div class="text-center py-5">
                                ...
                                <p class="text-muted">Anggota ini belum melakukan absensi kegiatan.</p>
                            </div>
                        @endif
                    </div>
                </div>
