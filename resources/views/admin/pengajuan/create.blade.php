        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0">Form Tambah Pengajuan</h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('pengajuan.store') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('judul_pengajuan') is-invalid @enderror" id="judul_pengajuan" name="judul_pengajuan" value="{{ old('judul_pengajuan') }}" required>
                            ...
                            @else
                                <select class="form-select @error('parent_id') is-invalid @enderror" id="parent_id" name="parent_id">
                                    ...
                                    @foreach($adminWilayahs as $admin)
                                        <option value="{{ $admin->id }}" {{ old('parent_id') == $admin->id ? 'selected' : '' }}>
                                            {{ $admin->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            @else
                                <select class="form-select @error('parent_id_cabang') is-invalid @enderror" id="parent_id_cabang" name="parent_id_cabang">
                                    ...
                                    @foreach($adminCabangs as $admin)
                                        <option value="{{ $admin->id }}" {{ old('parent_id_cabang') == $admin->id ? 'selected' : '' }}>
                                            {{ $admin->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('link_berkas') is-invalid @enderror" id="link_berkas" name="link_berkas" value="{{ old('link_berkas') }}" placeholder="https://example.com/file.pdf">
                            @error('link_berkas')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
