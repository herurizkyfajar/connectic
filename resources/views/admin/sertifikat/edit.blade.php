...
@section('page-title', 'EDIT SERTIFIKAT ANGGOTA')
            ...
            <form action="{{ route('admin.sertifikat.update', $sertifikat->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-section">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select select2 @error('anggota_id') is-invalid @enderror" 
                            ...
                            @foreach($anggotas as $anggota)
                                <option value="{{ $anggota->id }}" {{ (old('anggota_id', $sertifikat->anggota_id) == $anggota->id) ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select select2 @error('riwayat_kegiatan_id') is-invalid @enderror" 
                            ...
                            <option value="{{ $sertifikat->riwayat_kegiatan_id }}" selected>
                                {{ $sertifikat->riwayatKegiatan->judul }} ({{ $sertifikat->riwayatKegiatan->tanggal_kegiatan_formatted }})
                            </option>
                        </select>
                        @error('riwayat_kegiatan_id')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <small class="text-muted">Ketik minimal 2 karakter untuk mencari kegiatan lain</small>
                    </div>

                    <div id="kegiatan_info" class="alert alert-info mb-3">
                        <h6><i class="fas fa-info-circle me-2"></i>Informasi Kegiatan:</h6>
                        <div id="kegiatan_details">
                            <div class="row">
                                <div class="col-md-6">
                                    <p class="mb-1"><strong>Tanggal:</strong> {{ $sertifikat->riwayatKegiatan->tanggal_kegiatan_formatted }}</p>
                                    <p class="mb-1"><strong>Lokasi:</strong> {{ $sertifikat->riwayatKegiatan->lokasi }}</p>
                                </div>
                                <div class="col-md-6">
                                    <p class="mb-1"><strong>Jenis:</strong> <span class="badge bg-info">{{ $sertifikat->riwayatKegiatan->jenis_kegiatan }}</span></p>
                                    <p class="mb-1"><strong>Penyelenggara:</strong> {{ $sertifikat->riwayatKegiatan->penyelenggara }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="form-section">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('keterangan') is-invalid @enderror" 
                                  ...
                                  placeholder="Keterangan tambahan (opsional)">{{ old('keterangan', $sertifikat->keterangan) }}</textarea>
                        @error('keterangan')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="form-section">
                    ...
                    <div class="mb-3">
                        ...
                        <small class="text-muted">Format: PDF, JPG, PNG. Maksimal 2MB. Kosongkan jika tidak ingin mengubah</small>
                        @if($sertifikat->file_sertifikat)
                            <div class="mt-2">
                                <small class="text-muted">File saat ini: {{ basename($sertifikat->file_sertifikat) }}</small>
